  ...
  try {
    ...
    return await Promise.all(
      list.map(async (path) => {
        let packs = [];
        let rank;
        if (path.startsWith(benchmarker)) {
          rank = null;
          currentRank = 0;
        } else {
          rank = ++currentRank;
        ...
        try {
          const levelResult = await fetch(
            `${dir}/${
              path.startsWith(benchmarker) ? path.substring(1) : path
            }.json`
          );
    ...
    if (level.packs.length > 0) {
      level.packs.forEach((pack) => {
        if (Array.isArray(pack.levels)) {
          const allVerified = pack.levels.every((packLevel) =>
            list.some(
              ([_, __, lvl]) =>
                ...
                (lvl.records.some(
                  (r) =>
                    r.user.toLowerCase() === verifier.toLowerCase() &&
                    r.percent === 100
                ) ||
          ...
          const completedInDifficulty = list.filter(
            ([_, __, lvl]) =>
              lvl.difficulty >= 7 &&
              (lvl.records.some(
                (r) =>
              ...
              packs.some(
                (otherPack) =>
                  otherPack.levels &&
                  !otherPack.levels.some(
                    (otherLevel) => otherLevel.path === lvl.path
                  )
              )
          ).length;
                ...
                list.some(
                  ([_, __, lvl]) =>
                    ...
                    (lvl.records.some(
                      ...
                      lvl.verifier.toLowerCase() === user.toLowerCase())
                )
              );
              if (allCompleted) {
...
export async function fetchPacks(list) {
  try {
    ...
    users.forEach((user) => {
      let userLower = user.user ? user.user.toLowerCase() : user.toLowerCase(); // normalized user for comparison
      packs.forEach(async (pack) => {
        ...
        if (pack.levels) {
          ...
          const allCompleted = pack.levels.every((packLevel) =>
            list.some(
              ([_, __, lvl]) =>
                lvl.path == packLevel.path &&
                (lvl.records.some(
                  (r) => r.user.toLowerCase() === userLower && r.percent === 100
                ) ||
                  lvl.verifier.toLowerCase() === userLower)
            )
          );
          ...
          let levelsInDifficulty = list.filter(
            ([_, __, lvl]) => lvl.difficulty === pack.difficulty && lvl.id !== 0
          );
...
export async function fetchStaff() {
  try {
    ...
    for (const editor of staff) {
      if (!editor.link)
        editor.link =
          "https://anylist.pages.dev/#/leaderboard/user/" +
...
export async function fetchUsers() {
  ...
  const uniqueUsers = Array.from(
    new Set(userArray.map((user) => user.toLowerCase()))
  );
